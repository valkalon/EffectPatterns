...
```typescript
import { Effect, Logger, LogLevel, References, Cause } from "effect"
  ...
  yield* failingEffect.pipe(
    Effect.catchCause((cause) =>
      Effect.gen(function* () {
        ...
        // Extract all failures (v4: Cause is a flat reasons array)
...
const showRuntimeMetrics = Effect.gen(function* () {
  ...
  // v4: FiberRef / Runtime<R> removed. Fiber-local state lives in Context.Reference.
  // Read built-in references directly, e.g. `yield* References.CurrentLogLevel`.
  const level = yield* References.CurrentLogLevel
  yield* Effect.log(`Current log level: ${level.label}`)
  void services
})
