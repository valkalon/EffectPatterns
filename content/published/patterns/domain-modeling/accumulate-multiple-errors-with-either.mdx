...
When you need to perform multiple validation checks and collect all failures, use the `Result<E, A>` data type. `Result` represents a value that can be one of two possibilities: a `Failure<E>` (typically for failure) or a `Success<A>` (typically for success).
...
`Result` is the solution. Since it's a pure data structure, you can run multiple checks that each return a `Result`, and then combine the results to accumulate all the `Failure` (error) values. The `Effect/Schema` module uses this pattern internally to provide powerful error accumulation.
