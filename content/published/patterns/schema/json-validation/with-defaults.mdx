...
const ServerConfig = Schema.Struct({
  ...
  timeout: Schema.Number.pipe(
    ...
    Schema.withDecodingDefaultKey(Effect.succeed(5000)) // 5 second default
  ),
  ...
  compressionLevel: Schema.Number.pipe(
    ...
    Schema.withDecodingDefaultKey(Effect.succeed(6)) // Mid-range compression
  ),
});
...
| Concept | Explanation |
|---------|-------------|
| `Schema.withDecodingDefaultKey(Effect.succeed(value))` | Applies default when field is missing |
