...
rule:
  description: "Use Option.match() and Result.match() for declarative pattern matching on optional and error-prone values"
---

## Guideline

When you need to handle `Option` or `Result` values, use the `.match()` combinator instead of imperative checks. The `.match()` method provides a declarative, exhaustive way to handle all cases (Some/None for Option, Right/Left for Result) in a single expression.
